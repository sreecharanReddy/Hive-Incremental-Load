...
@patientid int = 22,							----For which patient you need to create appointments
@date datetime = '2017-12-07 00:00:00.000',		----From which Date you need to create appointments
@dayGap int = 1,								----Approximate gap between consecutive appointments
/********************************************inputs********************************************/
...
BEGIN
		...
		*/
		...
		INSERT INTO [dbo].[tbl_PatientProductReferral]
           ([OrderId]
           ,[ReferralId]
           ,[ReferredBy]
           ,[PrescribedById]
           ...
           ,[ICDId]
           ,[PaymentTypeCode])
		SELECT @OrderId, 4, 'Self', @doctorid, 1,1,1, null, 'CSH'
...
SET @date = dateadd(dd, @dayGap +1+CAST(RAND()*90900 AS INT)%3, @Date);
